...
const Register = ()=>{
    ...
    const userRegisterData = {firstName,lastName,age,country,phoneNumber,date_of_birthday,email,password,role_id}
    const storeRegisterData = {title:storeTitle, logo:storeLogo, description:storeDescription,}

    const confirmRegister = async ()=>{
        try{
            const userRegisterResult = await axios.post("http://localhost:5000/users/register",userRegisterData)
            const newId = userRegisterResult.data.user.id
            setOwnerId(newId)

            if (role_id === 2){const storeRegisterResult  = await axios.post("http://localhost:5000/stores/addnewstore",{...storeRegisterData,owner_id:newId})
            console.log(storeRegisterResult.data);
        }
    ...
    return (
        <div>
            ...
            {asOwner === true && <input placeholder="Store Name" onChange={(e)=>{setStoreTitle(e.target.value)}} /> }
            {asOwner === true && <input placeholder="About Store" onChange={(e)=>{setStoreDescription(e.target.value)}}/> }
            {asOwner === true && <input placeholder="Link for Store Logo" onChange={(e)=>{setStoreLogo(e.target.value)}} /> }
